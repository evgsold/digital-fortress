    ...
    <div className="min-h-screen bg-[#F7FAFC] text-[#718096]">
      ...
      <div className="bg-white border-b-2 border-[#E2E8F0]">
        <div className="container mx-auto px-4 py-6">
          ...
          <h1 className="text-3xl font-bold font-mono text-[#2D3748]">{t('create.title')}</h1>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          <motion.div
            ...
            className="bg-white border-2 border-[#E2E8F0] p-8 rounded-xl shadow-sm"
          >
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              {typeof window !== 'undefined' && localStorage.getItem('forum-draft') && (
                <div className="flex items-center gap-2 text-[#4299E1] bg-[#4299E1]/10 border border-[#4299E1]/30 px-4 py-3 rounded-lg">
              ...
              {/* Title */}
              <div>
                <label className="block text-lg font-bold mb-2 font-mono text-[#2D3748]">
                  {t('create.titleLabel')} *
                </label>
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-4 py-3 pr-10 bg-[#F7FAFC] text-[#2D3748] border-2 focus:outline-none font-mono rounded-lg placeholder-[#718096]/50 ${
                      errors.title ? 'border-red-500' : 
                      validationStatus.title === 'valid' ? 'border-[#4299E1]' :
                      'border-[#E2E8F0] focus:border-[#4299E1] focus:ring-1 focus:ring-[#4299E1]'
                    }`}
                  />
                  {validationStatus.title === 'valid' && (
                    <CheckCircle className="absolute right-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-[#4299E1]" />
                  )}
                </div>
                {errors.title && (
                  <p className="text-red-500 text-sm mt-1 font-mono">{errors.title}</p>
                )}
                <p className="text-[#718096]/80 text-sm mt-1 font-mono">
              ...
              <div>
                ...
                <select
                  ...
                  className={`w-full px-4 py-3 bg-[#F7FAFC] text-[#2D3748] border-2 focus:outline-none font-mono rounded-lg ${
                    errors.categoryId ? 'border-red-500' : 'border-[#E2E8F0] focus:border-[#4299E1] focus:ring-1 focus:ring-[#4299E1]'
                  }`}
                >
                    ...
                    <button
                      ...
                      className={`p-4 text-left border-2 transition-all duration-200 rounded-lg ${
                        formData.scamType === type.value
                          ...
                          : 'bg-transparent text-[#718096] border-[#E2E8F0] hover:bg-[#F7FAFC] hover:border-[#4299E1]'
                      }`}
                    >
              ...
              <div>
                ...
                <div className="relative">
                  <textarea
                    ...
                    rows={12}
                    className={`w-full px-4 py-3 pr-10 bg-[#F7FAFC] text-[#2D3748] border-2 focus:outline-none font-mono resize-none rounded-lg placeholder-[#718096]/50 ${
                      errors.content ? 'border-red-500' : 
                      validationStatus.content === 'valid' ? 'border-[#4299E1]' :
                      'border-[#E2E8F0] focus:border-[#4299E1] focus:ring-1 focus:ring-[#4299E1]'
                    }`}
                  />
                  {validationStatus.content === 'valid' && (
                    <CheckCircle className="absolute right-3 top-3 w-5 h-5 text-[#4299E1]" />
                  )}
                </div>
                {errors.content && (
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  <input
                    ...
                    className="flex-1 px-3 py-2 bg-[#F7FAFC] text-[#2D3748] border-2 border-[#E2E8F0] focus:border-[#4299E1] focus:ring-1 focus:ring-[#4299E1] focus:outline-none font-mono rounded-lg placeholder-[#718096]/50"
                  />
                  <button
                    type="button"
                    onClick={addTag}
                    className="px-4 py-2 bg-[#4299E1] text-white hover:bg-[#3182CE] font-mono font-bold border-2 border-[#4299E1] flex items-center gap-2 rounded-lg transition-colors"
                  >
                ...
                {formData.tags.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-2">
                    {formData.tags.map((tag: string, index: number) => (
                      <span
                        key={index}
                        className="px-3 py-1 bg-[#4299E1]/10 text-[#4299E1] font-mono text-sm flex items-center gap-2 rounded-md"
                      >
                ...
                <p className="text-[#718096]/80 text-sm font-mono">
                  {t('create.tagsHint')}
                </p>
              </div>
              ...
              <div>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 rounded text-[#4299E1] bg-[#F7FAFC] border-[#E2E8F0] focus:ring-[#4299E1]"
                  />
                  <div className="flex items-center gap-2">
                    <Shield className="w-5 h-5 text-[#4299E1]" />
                    <span className="font-bold font-mono text-[#2D3748]">{t('create.markResolved')}</span>
                  </div>
                </label>
                <p className="text-[#718096]/80 text-sm mt-1 ml-8 font-mono">
                  {t('create.markResolvedHint')}
                </p>
              </div>

              {/* Submit Button */}
              <div className="flex justify-end gap-4 pt-6 border-t-2 border-[#E2E8F0]">
                <Link
                  href="/forum"
                  className="px-6 py-3 text-[#718096] bg-transparent hover:bg-[#F7FAFC] font-mono border-2 border-[#E2E8F0] rounded-lg transition-colors"
                >
                  {t('common.cancel')}
                </Link>
                <button
                  ...
                  className="px-8 py-3 bg-[#4299E1] text-white hover:bg-[#3182CE] disabled:bg-[#CBD5E0] disabled:text-[#718096] disabled:border-transparent disabled:cursor-not-allowed font-mono font-bold border-2 border-[#4299E1] flex items-center gap-2 transition-all duration-300 rounded-lg"
                >
                  {isSubmitting ? (
                    <>
