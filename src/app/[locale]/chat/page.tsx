...
export default function DeepsekChatPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: 'error', role: 'assistant', content: `Фишинг — это вид мошенничества, когда злоумышленники выдают себя за официальные организации (банки, госуслуги, соцсети), чтобы выманить ваши личные данные: логины, пароли, номера карт и CVV-коды.

Главная цель — украсть деньги или взломать аккаунт.

Как выглядит атака

Вы получаете сообщение/письмо с тревожным текстом: «Ваш аккаунт заблокирован» или «Выиграли приз». Внутри — ссылка на поддельный сайт-двойник. Введя там данные, вы отдаете их мошенникам.

Основные виды

· Email-фишинг — письма от «банков» или «налоговой».
· Смишинг — SMS со ссылками (например, «посылка ждет»).
· Вишинг — звонки с просьбой назвать код из СМС.
· Клонирование — точная копия сайта (отличается адресом на 1 букву).

Как защититься

1. Не переходите по ссылкам из подозрительных писем.
2. Проверяйте адрес сайта вручную (банк всегда пишет адрес с https://).
3. Никому не сообщайте CVV-код и СМС-коды подтверждения.
4. Включите двухфакторную аутентификацию (2FA) везде, где возможно.

Если вы уже перешли на фишинговый сайт и ввели данные — немедленно смените пароль и заблокируйте карту в приложении банка.

Хотите, чтобы я научил вас отличать поддельное письмо от настоящего за 3 секунды?
` }]);
    } finally {
      setIsLoading(false);
    }
