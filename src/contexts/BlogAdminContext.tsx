  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [a, c, p] = await Promise.all([
        blogAuthorOperations.list().catch(e => {
          console.error('Error loading authors:', e);
          return [];
        }),
        blogCategoryOperations.list().catch(e => {
          console.error('Error loading categories:', e);
          return [];
        }),
        blogPostOperations.list().catch(e => {
      ...
      setError(errorMsg);
      throw new Error(errorMsg); // Re-throw to allow error boundaries to catch it
    } finally {
      setLoading(false);
    }
  }, []);
